...
`use_bio_tooltips()` uses vendored `bio-tooltips` 2.3.1, D3 7.9.0, and
...
reliable pattern is to let Plotly handle the plot hover (and click, for touch)
events and update a real HTML Bio Tooltip span next to the plot.
...
volcano_plot <- plotly::plot_ly(
  ...
  marker = list(size = 12, opacity = 0.82, line = list(width = 0)),
...
Plotly still owns the pointer events, but its default hover card is suppressed.
`bt_plotly_gene_hover()` reads the gene symbol from Plotly's `key` field and
opens a Bio Tooltip for it. Hover over a point on desktop, or tap a point on a
touch device: on narrow screens (600 CSS pixels or less) the tooltip opens as a
persistent bottom drawer that stays open until another point is selected or the
user dismisses it.
