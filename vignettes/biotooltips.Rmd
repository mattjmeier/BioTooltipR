...
volcano$tooltip_html <- as.character(gene_tt(volcano$symbol, species = "human"))
  ...
  htmlwidgets::onRender(
    ...
    function(el) {
      ...
      function activeTarget() {
        var wrapper = el.closest('.bt-volcano-demo');
        return wrapper ? wrapper.querySelector('.bt-volcano-cursor-gene') : null;
      }
      ...
      el.on('plotly_hover', function(eventData) {
        ...
        if (!target || !point || !point.customdata) return;

        target.innerHTML = point.customdata;
...
htmltools::tagList(
  htmltools::tags$style(htmltools::HTML("
    .bt-volcano-demo .bt-volcano-cursor-gene {
      position: fixed;
      width: 1px;
      height: 1px;
      overflow: hidden;
    ...
    .bt-volcano-demo .bt-volcano-cursor-gene .gene-tooltip {
  ...
  ")),
  htmltools::tags$div(
    class = "bt-volcano-demo",
    volcano_plot,
    htmltools::tags$span(class = "bt-volcano-cursor-gene")
  )
)
}
```
...
The event updates a tiny fixed-position `gene_tt()` span at the cursor so Bio
Tooltips can use its usual DOM-based positioning.
